...
var OXYGEN_OPTIONS = {
  ...
  provider: 'autodesk',
  providers_endpoint: {
    autodesk: 'https://accounts-staging.autodesk.com'
  },
  ...
  var Oxygen = this.Oxygen = (function() {
        ...
        fn = {
          ...
          getBaseOpenIDProviderURL: function(provider, immediate) {
            var providerURL = options.providers_endpoint[provider] + "/SignIn" +
                              ...
                              "&openid.ax.required=name,profileImg,oxygenId" +
                              "&openid.ax.mode=fetch_request" +
                              "&openid.ax.type.name=" + encodeURIComponent("http://axschema.org/namePerson") +
                              "&openid.ax.type.profileImg=" + encodeURIComponent("http://axschema.org/autodesk/media/image/50") +
                              "&openid.ax.type.oxygenId=" + encodeURIComponent("http://axschema.org/autodesk/userid") +
    ...
    return {
      ...
      checkImmediate: function(url) {
        ...
        var iframe = OxygenIframe(url || fn.getBaseOpenIDProviderURL(options.provider, true), options.onAfterCheckImmediate);
